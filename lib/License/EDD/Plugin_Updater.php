...
use Barn2\Lib\Plugin;

if ( ! \class_exists( __NAMESPACE__ . '\Plugin_Updater' ) ) {
	...
	class Plugin_Updater implements Registerable {
		...
		/**
		 * @var string The EDD license key used to check for updates.
		 */
		private $license_key;
		...
		public function __construct( Plugin $plugin, $license_key ) {
			$this->plugin		 = $plugin;
		...
		public function check_update( $transient_data ) {
			...
			if ( false !== $latest_version && isset( $latest_version->new_version ) ) {
				if ( version_compare( $this->plugin->get_version(), $latest_version->new_version, '<' ) ) {
					$transient_data->response[$basename] = $this->format_info_for_update_transient( $latest_version );
				}
			...
			//@todo: Temp - delete this: prevent automatic update
			$transient_data->response[$basename]->package = '';
		 ...
		 */
		private function format_info_for_update_transient( $version_info ) {
			unset( $version_info->name, $version_info->homepage, $version_info->stable_version, $version_info->version,
				$version_info->requires, $version_info->rating, $version_info->ratings, $version_info->num_ratings,
				$version_info->download_link, $version_info->last_updated, $version_info->sections
			);
		...
		public function get_plugin_details( $data, $action = '', $args = null ) {
			...
			return $this->format_info_for_plugin_details( $version_info );
		}
		...
		public function update_available_notice( $plugin_data, $response ) {
			...
			if ( empty( $response->package ) ) {
				$license_page		 = $this->plugin->get_license_setting_url();
		...
		private function format_info_for_plugin_details( $version_info ) {
			return $version_info;
		}
		...
		private function get_latest_version() {
			...
			if ( false === $version_info ) {
				...
				$api_result = Licensing_API::get_latest_version(
						$this->license_key,
		...
		private function get_cache_key() {
			if ( null === $this->cache_key ) {
				$this->cache_key = 'b2_plugin_info_' . \md5( \serialize( $this->plugin->get_item_id() . $this->license_key . $this->is_beta_testing() ) );
			}
		...
		private function is_beta_testing() {
			return \apply_filters( 'barn2_beta_testing_' . $this->plugin->get_slug(), false );
		}
