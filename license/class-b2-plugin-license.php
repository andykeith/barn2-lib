	...
	class Barn2_Plugin_License {
		...
		public function get_license_key_admin_message() {
			...
			if ( $this->is_valid() ) {
				...
				$message = __( 'License key successfully deactivated. Click Save to reactivate.', 'barn2' );
			} elseif ( $this->failed_deactivation() ) {
				$message = sprintf( '<span style="color:red;">%s</span>', __( 'There was an error deactivating your license key, please try again.', 'barn2' ) ) . $deactivate_button;
			} elseif ( $this->is_invalid() ) {
				$message = $this->get_license_error();
				if ( ! $message ) {
					$message = __( 'There was an error activating your license, please try again.', 'barn2' );
				}
